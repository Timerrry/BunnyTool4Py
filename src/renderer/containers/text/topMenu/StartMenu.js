...
import { toggle as toggleFileMenu, switchTab, toggleFilelist } from '../../../reducers/text/fileMenu';
import { toggle as toggleLibraryManager } from '../../../reducers/text/libraryManager';
import { setBoard } from '../../../reducers/text/project';
import { setPort, setPorts, setFileList, setPortId } from '../../../reducers/text/config';
import { setBuildMessage, setBuildAndUploadLock, setBuildError } from '../../../reducers/text/build';
...
import { stamp, rejectError } from '../../../lib/util';
...
class StartMenu extends Component {
	...
	_handleOpenSerialPort() {
		...
		return bridge.postMessage("serialPort.open", comName, { baudRate: 115200, parser: "raw" })
			...
			.catch(err => {
				message.error("串口打开失败，上传失败（Error 1）");
			});
	}
	...
	_handleSerialPortStatus() {
		...
		return bridge.postMessage("serialPort.write", portId, "\r\n")
			...
			.catch(err => {
				console.log(err);
				message.error("串口打开失败，上传失败（Error 2）");
			});
	}
